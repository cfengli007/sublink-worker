...
import { groupProxiesByCountry, parseCountryFromNodeName } from '../utils.js';
...
export class SurgeConfigBuilder extends BaseConfigBuilder {
    ...
    addCountryGroups() {
        // === 插入点 1：前置过滤 ===
        let proxies = this.getValidProxies();
        ...
        const manualProxyNames = allProxies.map(p => this.getProxyName(p)).filter(Boolean);
        ...
        if (otherCountryNodes.length > 0) {
            ...
            if (!existing.has(otherGroupName.trim())) {
                this.config['proxy-groups'].push(
                    this.createProxyGroup(otherGroupName, 'select', otherCountryNodes)
                );
                existing.add(otherGroupName.trim());
            }
